...
locus.zoom2 <- function(BP = NULL, P = NULL, SNP.List = NULL, SNP = NA, LD.Matrix = NULL, Chr = NULL, Genes.Data = NULL, Plot.Title = NULL, Nominal = 6, Significant = 7.3, File.Name = NULL){
  # Load Data
  ...
  new.ld.data <- read.table("~/GIT_Repos/GenomicPrediction/locuszoom2.ld",header=T)
  head(new.ld.data)
  new.ld.data <- new.ld.data[,c("SNP_B","R2")]
  colnames(new.ld.data) <- c("snps",SNP)
  #colnames(new.ld.data) <- c("snps","rs11583200")
  ...
  p.max <- -log10(p.min)
  ymax <-  max(round.up(p.max, decimals = 0), 8.5)
  ...
  plot(x = new.results.data.plot[,"pos"], y = -log10(new.results.data.plot[,"p"]), ylim = c(0, ymax), pch = 20, col = as.character(new.results.data.plot[,"Colour"]), las = 2, xlab = "", ylab = expression(-log[10](italic(P))), cex = 1.5, xaxt = "n")
  ...
  for(gene in 1:length(genes.data[,"Gene"])){
    ...
    text(x = (genes.data[gene,"Start"] + genes.data[gene,"End"])/2, y = genes.data[gene, "Y"] + 0.2, labels = genes.data[gene, "Gene"], cex = 0.6)
  }
  dev.off()
}


####set up list of variables for the functions
trait <- "BMI"
cohort <- "west"
covar <- ""
curve <- "linear"
window <- 2 ### 1Mb window, so +/- 499999bp

### read the results
data <- read.table(paste0("~/GIT_Repos/GenomicPrediction/as",trait,cohort,covar,".assoc.",curve),header=T)
data <- data[data$TEST=="ADD",]
genes.all <- read.table(file="~/GIT_Repos/GenomicPrediction/Data/dimGENE.txt",header=T,sep=",")


locuszoomsnp <- function(x){
  ### snp info
  #snp.name <- "exm-rs7096169"
  snp.name <- x
  
  snp.chr <- data[data$SNP==snp.name,"CHR"]
  snp.pos <- data[data$SNP==snp.name,"BP"] 
  
  ### trim data to plotting data
  data.plot <- subset(data,data$CHR==snp.chr & data$BP>=(snp.pos-(window*999999)/2 & data$BP<=(snp.pos+(window*999999)/2)))
  
  ### only start the plot if the current snp is the best snp in the range
  best <- data.plot[which(data.plot$P == min(data.plot$P)),"SNP"]
  if(best==snp.name){
    
    ### make the list of snps for the locus zoom plot
    snp.list <- data.plot$SNP
    
    genes.chr <- genes.all[genes.all$chromosome_name==snp.chr,]
    genes.chr <- genes.chr[,c(3,5,6)]
    colnames(genes.chr) <- c("Gene","Start","End")
    
    if(cohort=="east"){capscohort="East"}else{capscohort="West"}
    
    pcommand <- paste0("plink1 --bfile ~/GIT_Repos/GenomicPrediction/Data/Batch1-4_Merged_ExMiss5 ",
                       "--hwe 0.00001 --geno 0.05 --maf 0.01 ",
                       paste0("--keep ~/GIT_Repos/GenomicPrediction/Data/",capscohort,"PolynesianList.txt "),
                       paste0("--r2 --ld-window ",(window*999999)/2," "),
                       "--ld-window-r2 0 ",
                       paste0("--ld-snp ",snp.name," "),
                       "--out locuszoom2 ")
    print(paste0("Running Plink-LD for ",snp.name))
    system(pcommand,intern = T)
    
    locuszoom2.ld <- read.table("~/GIT_Repos/GenomicPrediction/locuszoom2.ld",header=T)
    write.table(locuszoom2.ld,file=paste0(cohort,trait,"_",snp.name,".ld"),quote=F,row.names=F)
    
    locus.zoom2(BP = data.plot$BP, P = data.plot$P, SNP.List = snp.list, LD.Matrix = locuszoom2.ld, 
                Chr = snp.chr, Genes.Data = genes.chr, Plot.Title =  paste0(capscohort," - ",trait,"\n",snp.name), File.Name = paste0(cohort,trait,"_",snp.name,".jpg"))
  }else{
    print(paste0(snp.name," is not the best SNP, next plot"))
  }
}
